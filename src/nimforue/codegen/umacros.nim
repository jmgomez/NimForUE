...
func funcBlockToFunctionInUClass(funcBlock : NimNode, ueTypeName:string) :  tuple[fws:seq[NimNode], impl:NimNode, metas:seq[UEMetadata], fnFields: seq[UEField]] = 
    let metas = funcBlock.childrenAsSeq()
                    ...
                    .filterIt(it.kind in {nnkIdent, nnkExprEqExpr, nnkExprColonExpr})
...
func genUFuncsForUClass*(body:NimNode, ueTypeName:string, nimProcs:seq[NimNode]): UFuncsInClass = 
  ...
  let fnBlocks = body.toSeq().filterFields(ValidUFuncs)
...
proc uClassImpl*(name:NimNode, body:NimNode, withForwards = true): (NimNode, NimNode, UFuncsInClass) = 
    ...
    #   members.add emissionAst
      ...
      let nimFields = body.children.toSeq.filterFields(@ValidUprops & @["default", "defaults"] & @ValidUFuncs, exclude = true)
...
macro uStruct*(name:untyped, body : untyped) : untyped = 
  ...
  let preFilteredNimFields = body.children.toSeq
    .filterIt(it.kind == nnkCall)
    .map(unwrapObjConstrForCall)

  for f in preFilteredNimFields:
    if f[0].strVal() in ValidUFuncs:
      error(&"{f.lineinfo} ufuncs are not allowed in uStructs: \nuStruct " & name.repr & "\n" & f.repr)

  let nimFields = preFilteredNimFields
    .filterIt(it[0].strVal() notin ValidUProps)
    .map(fromCallNodeToIdentDenf)
